...
C# bindings for **Cairo** — 2D vector drawing: paths, transforms, patterns, gradients, text and image surfaces.

In Gtk 4 you reach Cairo through a `Gtk.DrawingArea`'s draw function.

## What it binds

Native library: `libcairo`

Cairo has no usable introspection data, so **this binding is written entirely by hand**.

> Cairo objects are `IDisposable` and must be disposed. Leaking a `Cairo.Path`
> or surface does not merely warn — the finalizer takes the process down, at
> whatever moment the GC next runs.
